...
const items = [
  ...
  function filter(items, price) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (items[i].price === price) output.push(items[i])
    }
    return output
}
//////or
...
////Stretch
function filter(items, conditions) {
  return items.filter(item => {
    if (conditions.name && !item.name.includes(conditions.name)) return false
    if (conditions.price && item.price !== conditions.price) return false
    if (conditions.tags) {
      for (let tag of conditions.tags) {
        if (!item.tags.includes(tag.toLowerCase())) return false
      }
    }
    return true
  })
